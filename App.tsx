...
const App: React.FC = () => {
  ...
  const loadUserData = async (currentSession: UserSession) => {
    ...
    try {
      ...
      const fetchedGames: Game[] = data.games.map((g: any) => ({
        ...
        completionRate: 0, // Wird später via Achievements berechnet
        totalAchievements: 0,
        unlockedAchievements: 0,
        lastPlayed: new Date().toISOString()
      }));

      // In dieser Version laden wir Achievements on-demand oder für die Top-Spiele
      const achievementsMap: Record<string, Achievement[]> = {};
      
      // Lade Achievements für die Top 3 Spiele (Vorschau)
      for (const game of fetchedGames.slice(0, 3)) {
        ...
        // Sortiere Achievements nach Rarity (Einfach -> Schwer)
        achs.sort((a: any, b: any) => parseFloat(b.rarity) - parseFloat(a.rarity));
  ...
  const navigateToGame = async (id: string) => {
    // Lade Achievements falls noch nicht vorhanden
  ...
  return (
    ...
    >
      {isSyncing ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-10 animate-in fade-in h-full">
           <div className="relative">
              <div className="w-24 h-24 border-t-2 border-amber-500 rounded-full animate-spin"></div>
              <CpuChipIcon className="w-10 h-10 text-amber-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-[0_0_20px_rgba(245,158,11,0.5)]" />
           </div>
           <p className="text-amber-500 font-black uppercase tracking-[0.4em] text-[10px] italic">{syncStage}</p>
        </div>
      ) : activeTab === 'library' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-20 animate-in fade-in">
          {games.length === 0 ? (
            <div className="col-span-full py-32 text-center text-zinc-700 font-black uppercase tracking-[0.3em] text-[10px]">Library_Empty_Connect_To_Sync</div>
          ) : (
            games.map(g => (
              <div key={g.id} onClick={() => navigateToGame(g.id)} className="bg-zinc-900 border border-zinc-800 rounded-[2rem] overflow-hidden active:scale-[0.98] transition-all flex items-center p-5 gap-6 hover:border-amber-500/30">
                <img src={g.coverUrl} className="w-24 h-14 object-cover rounded-xl border border-zinc-800" alt="" />
                <div className="flex-1 text-left min-w-0">
                  <h4 className="font-black text-[11px] uppercase tracking-tighter truncate text-zinc-100">{g.title}</h4>
                  <div className="flex items-center space-x-3 mt-2">
                     <div className="h-1 bg-zinc-800 flex-1 rounded-full overflow-hidden">
                        <div className="h-full bg-amber-500" style={{ width: `${g.completionRate}%` }}></div>
                     </div>
                     <span className="text-[9px] font-mono font-black text-amber-500">{g.completionRate}%</span>
                  </div>
                </div>
              </div>
            ))
